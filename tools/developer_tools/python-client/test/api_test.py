...
class MyTestCase(unittest.TestCase):
    ...
    def test_create_logbook_document(self):
        doc_name = self.__gen_unique_name() + "Basic"
        ...
        existing_logbooks = self.logbook_api.get_log_documents(self.CTL_LOGBOOK_ID, limit=1000)

        new_log_document = self.logbook_api.create_logbook_document(log_document_options=options)
        self.assertIsNotNone(new_log_document.id)

        with self.assertRaises(OpenApiException, msg="Not failed creating a same document twice."):        
            self.logbook_api.create_logbook_document(log_document_options=options)

        new_logbooks = self.logbook_api.get_log_documents(self.CTL_LOGBOOK_ID, limit=1000)
    ...
    def test_create_logbook_document_per_entry_template(self):
        doc_name = self.__gen_unique_name() + " Entry Per Template"
        options = LogDocumentOptions(name=doc_name, logbook_type_id=self.CTL_LOGBOOK_ID)
        options.template_id = self.DOC_TEMPLATE_PER_ENTRY_ID        

        template_entries = self.logbook_api.get_log_entries(self.DOC_TEMPLATE_PER_ENTRY_ID)
        ...
        self.assertEqual(entry_template.log_entry, expected_text, msg='Entry not fetched as expected')

        entry_template.log_entry += "\n\n" + doc_name
        new_entry = self.logbook_api.add_update_log_entry(entry_template)

        self.assertEqual(entry_template.log_entry, new_entry.log_entry, msg='new entry not matching added entry')

    def test_create_logbook_document_from_non_template_id(self):
        doc_name = self.__gen_unique_name() + " From non-template"
        options = LogDocumentOptions(name=doc_name, logbook_type_id=self.CTL_LOGBOOK_ID)
        options.template_id = self.TEST_DOCUMENT_ID        

        self.login_as_user()

        with self.assertRaises(OpenApiException, msg="Was able to create log document from non-template"):
            self.logbook_api.create_logbook_document(log_document_options=options)
    ...
    def test_create_from_template_with_copy(self):
        doc_name = self.__gen_unique_name() + " Copy Template"
    ...
    def test_create_from_template_with_sections_copy(self):
        doc_name = self.__gen_unique_name() + " Section Copy Template"
        ...
        template_sections = self.logbook_api.get_logbook_sections(self.DOC_TEMPLATE_SECTIONS_COPY_ID)
        new_log_document_sections = self.logbook_api.get_logbook_sections(new_log_document.id)
    ...
    def test_no_template_per_entry(self):
        doc_name = self.__gen_unique_name() + " No template per entry test"
    ...
    def test_add_log_entry_to_sample_log_document(self):
        ...
        log_entry_text = "sample entry text %s" % self.__gen_unique_name() 
    ...
    def test_create_log_document_with_sections(self):
        doc_name = self.__gen_unique_name() + " New doc with sections."
        ...
        added_sections = self.logbook_api.get_logbook_sections(log_document_id=new_doc.id)
